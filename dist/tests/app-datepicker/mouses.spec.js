...
describe('mouses', () => {
    ...
    it(`switches to calendar view`, async () => {
        ...
        const hasCalendarView = await browser.executeAsync(async (a, b, done) => {
            const n = document.body.querySelector(a);
            const calendarView = n.shadowRoot.querySelector(b);
    ...
    it(`focuses date after navigating away when switching to calendar view`, async () => {
        ...
        await clickElements([`.btn__calendar-selector`]);
        const [hasCalendarView, focusedDateContent,] = await browser.executeAsync(async (a, b, c, done) => {
            const n = document.body.querySelector(a);
            const root = n.shadowRoot;
            const calendarView = root.querySelector(b);
            const focusedDate = root.querySelector(c);
            done([
                calendarView != null,
                focusedDate.outerHTML,
            ]);
        }, DATEPICKER_NAME, '.datepicker-body__calendar-view', toSelector('.day--focused'));
    ...
    it(`switches back to calendar view when new year is selected`, async () => {
        ...
        const [prop, yearSelectorButtonContent, calendarLabelContent, calendarDaysContents,] = await browser.executeAsync(async (a, b, c, d, done) => {
            const n = document.body.querySelector(a);
            const root = n.shadowRoot;
            const yearSelectorButton = root.querySelector(b);
        ...
        }, DATEPICKER_NAME, '.btn__year-selector', toSelector('.calendar-label'), toSelector('.full-calendar__day'));
    ...
    it(`selects new focused date in current month`, async () => {
        ...
        const [prop, focusedDateContent,] = await browser.executeAsync(async (a, b, done) => {
            const n = document.body.querySelector(a);
    ...
    it(`selects new focused date in new month`, async () => {
        ...
        const [focusedDateContent, calendarLabelContent,] = await browser.executeAsync(async (a, b, c, done) => {
            const n = document.body.querySelector(a);
            const root = n.shadowRoot;
    ...
    it(`does not show months before 'min'`, async () => {
        ...
        const [noPrevMonthSelector, calendarLabelContent, calendarDaysContents,] = await browser.executeAsync(async (a, b, c, d, done) => {
            const n = document.body.querySelector(a);
            const root = n.shadowRoot;
            const prevMonthSelector = root.querySelector(b);
            const calendarLabel = root.querySelector(c);
            const calendarDays = Array.from(root.querySelectorAll(d), o => o.textContent);
            done([
        ...
        }, DATEPICKER_NAME, `.btn__month-selector[aria-label="Previous month"]`, toSelector('.calendar-label'), toSelector('.full-calendar__day'));
    ...
    it(`does not show months after 'max'`, async () => {
        ...
        const [noNextMonthSelector, calendarLabelContent, calendarDaysContents,] = await browser.executeAsync(async (a, b, c, d, done) => {
            const n = document.body.querySelector(a);
            const root = n.shadowRoot;
            const nextMonthSelector = root.querySelector(b);
            const calendarLabel = root.querySelector(c);
            const calendarDays = Array.from(root.querySelectorAll(d), o => o.textContent);
            done([
        ...
        }, DATEPICKER_NAME, `.btn__month-selector[aria-label="Next month"]`, toSelector('.calendar-label'), toSelector('.full-calendar__day'));
    ...
    it(`shows correct 'min' month when spam clicking previous month button`, async () => {
        await clickElements(Array.from('1234', () => (`.btn__month-selector[aria-label="Previous month"]`)), {
            props: {
                min: '2020-01-01',
                value: '2020-05-02',
            },
    ...
    it(`shows correct 'max' month when spam clicking next month button`, async () => {
        await clickElements(Array.from('1234', () => (`.btn__month-selector[aria-label="Next month"]`)), {
            props: {
                max: '2020-03-31',
                value: '2019-11-02',
            },
    ...
    const updateYear = async (newYear, prepareOptions) => {
        await clickElements([`.btn__year-selector`], prepareOptions);
        const yearIdx = await browser.executeAsync(async (a, b, c, done) => {
            const n = document.body.querySelector(a);
            const allYears = Array.from(n.shadowRoot.querySelectorAll(b), o => o.textContent);
    ...
    const queryCalendarLabel = async () => {
        const label = await browser.executeAsync(async (a, b, done) => {
            const n = document.body.querySelector(a);
    ...
    it(`resets month after updating 'value' and 'firstDayOfWeek'`, async () => {
        ...
        await clickElements(Array.from('12', () => `.btn__month-selector[aria-label="Next month"]`));
        const [calendarLabelContent3, calendarDaysContents,] = await browser.executeAsync(async (a, b, c, done) => {
            const n = document.body.querySelector(a);
            const root = n.shadowRoot;
            const calendarLabel3 = root.querySelector(b);
    ...
    it(`selects new focused date with optional 'locale'`, async () => {
        ...
        const [prop, focusedDateContent, calendarLabelContent, calendarDaysContents,] = await browser.executeAsync(async (a, b, c, d, done) => {
            const n = document.body.querySelector(a);
            const root = n.shadowRoot;
            const focusedDate = root.querySelector(b);
            const calendarLabel = root.querySelector(c);
        ...
        }, DATEPICKER_NAME, toSelector('.day--focused'), toSelector('.calendar-label'), toSelector('.full-calendar__day'));
        strictEqual(prop, '2020-02-13');
        strictEqual(cleanHtml(focusedDateContent), prettyHtml `
